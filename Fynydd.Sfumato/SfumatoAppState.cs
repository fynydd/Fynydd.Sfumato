...
#pragma warning restore SYSLIB1045
	...
	private async Task ProcessCliArgumentsAsync(IEnumerable<string>? args)
	{
		...
		if (index == -1)
			index = workingPath.IndexOf(Path.DirectorySeparatorChar + "Fynydd.Sfumato.Tests", StringComparison.InvariantCulture);

		index = workingPath[..index].TrimEnd(Path.DirectorySeparatorChar).LastIndexOf(Path.DirectorySeparatorChar);

		// Path.Combine(workingPath[..index], "Fynydd-Website-2024", "UmbracoCms");
